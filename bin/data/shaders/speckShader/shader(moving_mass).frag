...
void main(){
  ...
  // "moving mass (looks cool)"
  if(sum(nc) > sum(c)) {
    c = vec4(c.rgb*.5, c.a*0.8) + vec4(nc.rgb*.5, .5/(abs(sum(offset))+1) );
    //c.rgb *= smoothstep(0., .5, nc.a);
  }
  /*else if (sum(nc) == sum(c)) {
    c.rgb *= 0.9;
  }*/
  for(float x = -2; x <= 2; x++) {
    for(float y = -2; y <= 2; y++) {
      ...
      if(sum(bc) > .5) {
        float amount = .125/length(vec2(x, y));
        c = c*(1-amount) + bc*amount;
      }
    }
  }
  //if(length(offset) < 2) c = vec4(c.rgb , c.a*0.9);
  c.a = min(c.a, 1.);
  // dark colors get less alpha
  c.a *= smoothstep(0.1, .5, sum(c));
  //c.a *= smoothstep(0.2, 0.3, c.a);
	outColor = vec4(c);
}
